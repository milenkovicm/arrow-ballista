...
pub struct DistributedQueryExec<T: 'static + AsLogicalPlan> {
    ...
    // /// number of rows fetched by client
    // metric_row_count: Count,
    // // total bytes fetched by the client
    // metric_total_bytes: Count,
...
impl<T: 'static + AsLogicalPlan> ExecutionPlan for DistributedQueryExec<T> {
    ...
    fn execute(
        ...
        let metric_total_bytes =
            MetricBuilder::new(&self.metrics).counter("total bytes", partition);
